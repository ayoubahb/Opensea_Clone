...
<html lang="en">
	...
	<body>
		...
		<section class="nft">
			<div class="container">
				<div class="nfts-cards">
					<div>
						<h3 class="text-center">Most expensive NFT</h3>
						<?php
						$sql="SELECT nfts.img ,nfts.name,nfts.price,artist.artist_img FROM nfts JOIN collections ON nfts.coll_id=collections.id JOIN artist ON collections.artist_id=artist.id ORDER BY nfts.price DESC;";
					...
					<div>
						<h3 class="text-center">Cheapest NFT</h3>
						<?php
						$sql="SELECT nfts.img ,nfts.name,nfts.price,artist.artist_img FROM nfts JOIN collections ON nfts.coll_id=collections.id JOIN artist ON collections.artist_id=artist.id ORDER BY nfts.price;";
						$result=mysqli_query($con,$sql);
						if($row=mysqli_fetch_assoc($result)){
							$img=$row['img'];
							$name=$row['name'];
		...
		<!--  -->
		<h2 class="ttl mx-auto my-5">All NFTs</h2>
		<!--  -->
		<section class="nft">
			<div class="container">
				<div class="nfts-cards">
					<?php
					$sql="SELECT nfts.img ,nfts.name,nfts.price,artist.artist_img FROM nfts JOIN collections ON nfts.coll_id=collections.id JOIN artist ON collections.artist_id=artist.id";
						$result=mysqli_query($con,$sql);
						if($result){
